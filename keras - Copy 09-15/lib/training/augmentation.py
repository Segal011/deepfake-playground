...
class ImageAugmentation():
    ...
    def get_targets(self, batch):
        ...
        """
        logger.info("Compiling targets: batch shape: %s", batch.shape)
    ...
    def _separate_target_mask(target_batch):
        ...
        dict:
            ...
            masks available
        """
        logger.info("target_batch shapes: %s", [tgt.shape for tgt in target_batch])
        ...
        logger.info("returning: %s", {k: v.shape if isinstance(v, np.ndarray) else [tgt.shape
                                                                                     for tgt in v]
                                       for k, v in retval.items()})
        return retval
    ...
    def _random_clahe(self, batch):
        ...
        logger.info("Adjusting Contrast. Grid Sizes: %s", grid_sizes)
    ...
    def _random_lab(self, batch):
        ...
        logger.info("Random LAB adjustments: %s", randoms)
    ...
    def transform(self, batch):
        ...
        if self.is_display:
            return batch
        logger.info("Randomly transforming image")
        ...
        logger.info("Randomly transformed image")
        return batch
    ...
    def random_flip(self, batch):
        ...
        """
        if not self.is_display:
            logger.info("Randomly flipping image")
            ...
            logger.info("Randomly flipped %s images of %s", len(indices), self._batchsize)
        return batch
    ...
    def _random_warp(self, batch):
        ...
        logger.info("Randomly warping batch")
        ...
        logger.info("Warped image shape: %s", warped_batch.shape)
        return warped_batch
    ...
    def _random_warp_landmarks(self, batch, batch_src_points, batch_dst_points):
        ...
        logger.info("Randomly warping landmarks")
        ...
        logger.info("Warped batch shape: %s", warped_batch.shape)
        return warped_batch
    ...
    def skip_warp(self, batch):
        ...
        """
        logger.info("Compiling skip warp images: batch shape: %s", batch.shape)
        slices = self._constants["tgt_slices"]
        retval = np.array([cv2.resize(image[slices, slices, :],
                                      (self._input_size, self._input_size),
                                      cv2.INTER_AREA)
                           for image in batch], dtype='float32') / 255.
        logger.info("feed batch shape: %s", retval.shape)
        return retval

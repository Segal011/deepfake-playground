...
class Mask():  # pylint:disable=too-few-public-methods
    ...
    def _set_config(self,
        ...
        if config is None:
            retval = Config(section).config_dict
        else:
    ...
    def run(self,
        ...
        """
        logger.info("Performing mask adjustment: (detected_face: %s, "  # type: ignore
    ...
    def _get_mask(self,
        ...
        logger.info(mask.shape)  # type: ignore
        return mask
    ...
    def _get_erosion_kernels(self, mask: np.ndarray) -> List[np.ndarray]:
        ...
        logger.info("Erosion kernels: %s", [k.shape for k in kernels])  # type: ignore
        return kernels

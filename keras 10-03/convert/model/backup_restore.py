...
from convert.utils import get_folder
...
class Backup():
    ...
    def backup_model(full_path):
        ...
        if os.path.exists(full_path):
            logger.debug("Backing up: '%s' to '%s'", full_path, backupfile)
    ...
    def _move_archived(self):
        ...
        for filename in os.listdir(self.model_dir):
            if not self._check_valid(filename, for_restore=False):
                ...
                continue
            logger.debug("Moving '%s' to archived model folder: '%s'", filename, archive_dir)
            src = os.path.join(self.model_dir, filename)
            dst = os.path.join(archive_dir, filename)
            os.rename(src, dst)
        logger.debug("Archived existing model files")
        return archive_dir
    ...
    def _restore_files(self):
        ...
        for filename in os.listdir(self.model_dir):
            ...
            logger.debug("Restoring '%s' to '%s'", filename, dstfile)
            src = os.path.join(self.model_dir, filename)
            dst = os.path.join(self.model_dir, dstfile)
            copyfile(src, dst)
        logger.debug("Restored models from backup")

...
class ImagesLoader():
    ...
    def _load_extractor(self):
        ...
        """
        if not self._alignments.have_alignments_file and not True: #on the fly
    ...
    def _alignments_faces(self, frame_name, image):
        ...
        if not self._check_alignments(frame_name):
            return list()
        faces = self._alignments.data.get(frame_name, dict()).get("faces", [])
        # faces = self._alignments.get_faces_in_frame(frame_name)
        detected_faces = list()

        #image = cv2.imread(image_path)
        face_detector = DlibDetectedFace().detect_from_image(image)

...
class FfmpegReader(imageio.plugins.ffmpeg.FfmpegFormat.Reader):
    ...
    def get_frame_info(self, frame_pts=None, keyframes=None):
        ...
        while True:
            ...
            if "iskey" not in output:
                continue
            logger.info("Keyframe line: %s", output)
            ...
            logger.info("pts_time: %s, frame_no: %s", pts_time, frame_no)
    ...
    def _previous_keyframe_info(self, index=0):
        ...
        logger.info("keyframe pts_time: %s, keyframe: %s", prev_pts_time, prev_keyframe)
...
def read_image(filename, raise_error=False, with_metadata=False):
    ...
    """
    logger.info("Requested image: '%s'", filename)
    success = True
    image = None
    try:
        ...
        if raise_error:
            raise Exception(msg)
    logger.info("Loaded image: '%s'. Success: %s", filename, success)
    return retval
...
def read_image_batch(filenames, with_metadata=False):
    ...
    """
    logger.info("Requested batch: '%s'", filenames)
    ...
    logger.info("Returning images: (filenames: %s, batch shape: %s, with_metadata: %s)",
    ...
    with open(filename, "rb") as png, open(tmp_filename, "wb") as tmp:
        ...
        while True:
            ...
            if field == b"IDAT":  # Write out all remaining data
                logger.info("Writing image data and closing png")
                tmp.write(chunk + png.read())
                break

            if field != b"iTXt":  # Write non iTXt chunk straight out
                logger.info("Copying existing chunk")
            ...
            if keyword != b"faceswap":
                ...
                logger.info("Copying non-faceswap iTXt chunk: %s", keyword)
                tmp.write(keyword + b"\0" + value + png.read(4))
                continue

            logger.info("Updating faceswap iTXt chunk")
...
def png_read_meta(png):
    ...
    while True:
        ...
        if pointer < 0:
            logger.info("No metadata in png")
            break
...
def generate_thumbnail(image, size=96, quality=60):
    ...
    """
    logger.info("Input shape: %s, size: %s, quality: %s", image.shape, size, quality)
    ...
    logger.info("Output shape: %s", retval.shape)
    return retval
...
def batch_convert_color(batch, colorspace):
    ...
    """
    logger.info("Batch converting: (batch shape: %s, colorspace: %s)", batch.shape, colorspace)
...
class ImagesLoader(ImageIO):
    ...
    def _get_count_and_filelist(self, fast_count, count):
        ...
        logger.info("filelist: %s", self.file_list)
    ...
    def _process(self, queue):
        ...
        for retval in iterator():
            ...
            if image is None or (not image.any() and image.ndim not in (2, 3)):
                ...
                continue
            logger.info("Putting to queue: %s", [v.shape if isinstance(v, np.ndarray) else v
                                                  for v in retval])
            queue.put(retval)
        logger.info("Putting EOF")
        queue.put("EOF")
    ...
    def _from_video(self):
        ...
        for idx, frame in enumerate(reader):
            if idx in self._skip_list:
                logger.info("Skipping frame %s due to skip list", idx)
                continue
    ...
    def _from_folder(self):
        ...
        for idx, filename in enumerate(self.file_list):
            if idx in self._skip_list:
                logger.info("Skipping frame %s due to skip list")
                continue
    ...
    def load(self):
        ...
        while True:
            ...
            if retval == "EOF":
                logger.info("Got EOF")
                break
            logger.info("Yielding: %s", [v.shape if isinstance(v, np.ndarray) else v
                                          for v in retval])
            yield retval
...
class FacesLoader(ImagesLoader):
    ...
    def _from_folder(self):
        ...
        for idx, filename in enumerate(self.file_list):
            if idx in self._skip_list:
                logger.info("Skipping face %s due to skip list")
                continue
...
class SingleFrameLoader(ImagesLoader):
    ...
    def image_from_index(self, index):
        ...
        logger.info("index: %s, filename: %s image shape: %s", index, filename, image.shape)
...
class ImagesSaver(ImageIO):
    ...
    def _process(self, queue):
        ...
        while True:
            ...
            if item == "EOF":
                ...
                break
            logger.info("Submitting: '%s'", item[0])
    ...
    def save(self, filename, image):
        ...
        """
        self._set_thread()
        logger.info("Putting to save queue: '%s'", filename)

...
class TrainerBase():
    ...
    def _set_tensorboard(self):
        ...
        if self._model.state.current_session["no_logs"]:
            logger.debug("TensorBoard logging disabled")
            return None
        ...
        logger.debug("Enabled TensorBoard Logging")
        return tensorboard
    ...
    def train_one_step(self, viewer, timelapse_kwargs):
        ...
        * Creates a snapshot if the total iterations trained so far meet the requested snapshot \
        ...
        logger.info("Training one step: (iteration: %s)", self._model.iterations)
    ...
    def _log_tensorboard(self, loss):
        ...
        if not self._tensorboard:
            return
        logger.info("Updating TensorBoard log")
        logs = {log[0]: log[1]
    ...
    def _collate_and_store_loss(self, loss):
        ...
        logger.info("original loss: %s, comibed_loss: %s", loss, combined_loss)
...
class _Feeder():
    ...
    def get_batch(self):
        ...
        for side in ("a", "b"):
            ...
            logger.info("side: %s, input_shapes: %s, target_shapes: %s",
    ...
    def _compile_mask_targets(self, targets, masks, additional_masks):
        ...
        if not self._model.config["penalized_mask_loss"] and additional_masks is None:
            logger.info("No masks to compile. Returning targets")
            return targets
        ...
        logger.info("masks added to targets: %s", [tgt.shape for tgt in targets])
        return targets

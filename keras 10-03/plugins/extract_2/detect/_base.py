...
class Detector(Extractor):  # pylint:disable=abstract-method
    ...
    def get_batch(self, queue):
        ...
        if batch:
            ...
            logger.info("Returning batch: %s", {k: v.shape if isinstance(v, np.ndarray) else v
    ...
    def finalize(self, batch):
        ...
        if not isinstance(batch, dict):
            logger.info("Item out: %s", batch)
            return batch

        logger.info("Item out: %s", {k: v.shape if isinstance(v, np.ndarray) else v
                                      for k, v in batch.items()})
        ...
        for item in batch:
            ...
            logger.info("final output: (filename: '%s', image shape: %s, detected_faces: %s, "
    ...
    def _predict(self, batch):
        ...
        for angle in self.rotation:
            ...
            if angle != 0 and any([face.any() for face in batch["prediction"]]):
                logger.debug("found face(s) by rotating image %s degrees", angle)
            ...
            if all([face.any() for face in found_faces]):
                logger.info("Faces found for all images")
                break

        batch["prediction"] = found_faces
        logger.info("detect_prediction output: (filenames: %s, prediction: %s, rotmat: %s)",
    ...
    def _compile_detection_image(self, item):
        ...
        logger.info("compiled: (images shape: %s, scale: %s, pad: %s)", image.shape, scale, pad)
    ...
    def _set_scale(self, image_size):
        ...
        logger.info("Detector scale: %s", scale)
        return scale
    ...
    def _scale_image(image, image_size, scale):
        ...
        if scale != 1.0:
            ...
            logger.info("Resizing detection image from %s to %s. Scale=%s",
        ...
        logger.info("Resized image shape: %s", image.shape)
        return image
    ...
    def _pad_image(self, image):
        ...
        if width < self.input_size or height < self.input_size:
            ...
            image = cv2.copyMakeBorder(image,
                                       ...
                                       pad_r,
                                       cv2.BORDER_CONSTANT)
        logger.info("Padded image shape: %s", image.shape)
        return image
    ...
    def _rotate_face(face, rotation_matrix):
        ...
        """
        logger.info("Rotating face: (face: %s, rotation_matrix: %s)", face, rotation_matrix)
    ...
    def _rotate_image_by_angle(self, image, angle):
        ...
        logger.info("Rotating image: (image: %s, angle: %s)", image.shape, angle)
        ...
        logger.info("Rotated image: (rotation_matrix: %s", rotation_matrix)

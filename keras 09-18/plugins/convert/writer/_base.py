...
class Output():
    ...
    def output_filename(self, filename: str, separate_mask: bool = False) -> List[str]:
        ...
        logger.info("in filename: '%s', out filename: '%s'", filename, retval)  # type:ignore
        return retval
    ...
    def cache_frame(self, filename: str, image: np.ndarray) -> None:
        ...
        logger.info("Added to cache. Frame no: %s", frame_no)  # type: ignore
        logger.info("Current cache: %s", sorted(self.cache.keys()))  # type:ignore

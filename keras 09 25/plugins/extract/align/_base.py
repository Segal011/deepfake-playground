...
class Aligner(Extractor):  # pylint:disable=abstract-method
            ...
            for f_idx, face in enumerate(item.detected_faces):
                ...
                if idx == self.batchsize:
                    ...
                    if f_idx + 1 != frame_faces:
                        ...
                        logger.info("Rolled over %s faces of %s to next batch for '%s'",
    ...
    def _collect_item(self, queue):
        ...
        if self._rollover is not None:
            logger.info("Getting from _rollover: (filename: `%s`, faces: %s)",
            ...
            if item != "EOF":
                logger.info("Getting from queue: (filename: %s, faces: %s)",
    ...
    def finalize(self, batch):
        ...
        logger.info("Item out: %s", {key: val.shape if isinstance(val, np.ndarray) else val

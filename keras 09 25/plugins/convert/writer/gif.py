...
class Writer(Output):
    ...
    def write(self, filename: str, image) -> None:
        ...
        """
        logger.info("Received frame: (filename: '%s', shape: %s", filename, image.shape)
    ...
    def _save_from_cache(self) -> None:
        ...
        while self.frame_order:
            if self.frame_order[0] not in self.cache:
                logger.info("Next frame not ready. Continuing")
                break
            save_no = self.frame_order.pop(0)
            save_image = self.cache.pop(save_no)
            logger.info("Rendering from cache. Frame no: %s", save_no)
            self._writer.append_data(save_image[:, :, ::-1])
        logger.info("Current cache size: %s", len(self.cache))

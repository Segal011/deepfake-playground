...
class _Cache():
    ...
    def cache_metadata(self, filenames):
        ...
        with self._lock:
            ...
            logger.info("Needs cache: %s", needs_cache)
            ...
            if cache_full:
                logger.debug("Cache filled: '%s'", os.path.dirname(filenames[0]))
    ...
    def _add_aligned_face(self, filename, alignments, image_size):
        ...
        aligned_face = AlignedFace(detected_face.landmarks_xy,
                                   ...
                                   size=self._size,
                                   is_aligned=True)
        logger.info("Caching aligned face for: %s", filename)
    ...
    def _add_mask(self, filename, detected_face):
        ...
        logger.info("Caching mask for: %s", filename)
    ...
    def _add_localized_mask(self, filename, detected_face, area):
        ...
        logger.info("Caching localized '%s' mask for: %s", key, filename)
...
class TrainingDataGenerator():  # pylint:disable=too-few-public-methods
    ...
    def _process_batch(self, filenames, side):
        ...
        """
        logger.info("Process batch: (filenames: '%s', side: '%s')", filenames, side)
        ...
        logger.info("Processed batch: (filenames: %s, side: '%s', processed: %s)",
                     filenames,
                     side,
    ...
    def _crop_to_center(self, filenames, cache, batch, side):
        ...
        FaceswapError
            ...
            the batch
        """
        logger.info("Cropping training images info: (filenames: %s, side: '%s')", filenames, side)
    ...
    def _apply_mask(self, filenames, cache, batch, side):
        ...
        """
        logger.info("Input filenames: %s, batch shape: %s, side: %s",
        ...
        for key in ("mask", "mask_eyes", "mask_mouth"):
            ...
            if lookup is None and key == "mask":
                logger.info("Creating dummy masks. side: %s", side)
                masks = np.ones_like(batch[..., :1], dtype=batch.dtype)
            else:
                logger.info("Obtaining masks for batch. (key: %s side: %s)", key, side)

                masks = np.array([self._get_mask(item[key], size)
                                  for item in cache], dtype=batch.dtype)
                masks = self._resize_masks(size, masks)
            logger.info("masks: (key: %s, shape: %s)", key, masks.shape)
            batch = np.concatenate((batch, masks), axis=-1)
        logger.info("Output batch shape: %s, side: %s", batch.shape, side)
        return batch
    ...
    def _resize_masks(cls, target_size, masks):
        ...
        logger.info("target size: %s, masks shape: %s", target_size, masks.shape)
        mask_size = masks.shape[1]
        if target_size == mask_size:
            logger.info("Mask and targets the same size. Not resizing")
            return masks
        ...
        masks = np.array([cv2.resize(mask,
                          ...
                          for mask in masks])
        logger.info("Resized masks: %s", masks.shape)
        return masks
    ...
    def _get_closest_match(self, filenames, side, batch_src_points):
        ...
        logger.info("Retrieving closest matched landmarks: (filenames: '%s', src_points: '%s'",
        ...
        logger.info("Returning: (batch_dst_points: %s)", batch_dst_points.shape)
    ...
    def _cache_closest_matches(self, filenames, batch_src_points, landmarks):
        ...
        logger.info("Caching closest matches")
        ...
        logger.info("Cached closest matches")

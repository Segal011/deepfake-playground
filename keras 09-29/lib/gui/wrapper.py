...
class FaceswapControl():
            ...
            if output:
                ...
                if self.command == "train" and self.wrapper.tk_vars["istraining"].get():
                    if "[saved models]" in output.strip().lower():
                        ...
                        logger.info("tk_vars: %s", {itm: var.get()
    ...
    def capture_loss(self, string):
        ...
        logger.info("Capturing loss")
        if not str.startswith(string, "["):
            logger.info("Not loss message. Returning False")
            return False
        ...
        if not message:
            logger.info("Error creating loss message. Returning False")
            return False
        ...
        logger.info("Succesfully captured loss: %s", message)
        return True
    ...
    def capture_tqdm(self, string):
        ...
        logger.info("Succesfully captured tqdm message: %s", message)
        return True

    def capture_ffmpeg(self, string):
        """ Capture tqdm output for progress bar """
        logger.info("Capturing ffmpeg")
        ffmpeg = self.consoleregex["ffmpeg"].findall(string)
        if len(ffmpeg) < 7:
            logger.info("Not ffmpeg message. Returning False")
            return False
        ...
        if not message:
            logger.info("Error creating ffmpeg message. Returning False")
            return False

        self.statusbar.progress_update(message, 0, False)
        logger.info("Succesfully captured ffmpeg message: %s", message)
        return True

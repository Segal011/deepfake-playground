...
from convert.model.backup_restore import Backup
from convert.utils import FaceswapError, get_backend
...
class Weights():
    ...
    def _check_weights_file(cls, weights_file: str) -> Optional[str]:
        ...
        logger.debug("Using weights file: %s", weights_file)  # type:ignore
        return weights_file
    ...
    def _load_layer_weights(self,
        ...
        if old_weights[0].shape != new_weights[0].shape:
            logger.warning("The weights for layer '%s' are of incompatible shapes. Skipping.",
                           layer.name)
            return 0
        logger.debug("Setting weights for '%s'", layer.name)  # type:ignore

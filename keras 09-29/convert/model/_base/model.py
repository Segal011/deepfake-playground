...
from convert.model.nn_blocks import set_config as set_nnblock_config
from convert.utils import get_backend, FaceswapError
...
class ModelBase():
    ...
    def _output_summary(self) -> None:
        ...
        if hasattr(self._args, "summary") and self._args.summary:
            ...
            # print to logger
            print_fn = lambda x: logger.info("%s", x)  # type: ignore # noqa
...
class State():
    ...
    def __init__(self,
        ...
        filename = f"{model_name}_state.json"
    ...
    def _load(self, config_changeable_items: dict) -> None:
        ...
        if not os.path.exists(self._filename):
            ...
            return
        try:
            with open(self._filename, "rb") as s_file:
                data = s_file.read()
                logger.debug("stored data type: %s", type(data))
                state = json.loads(data.decode("utf-8"))
        except IOError as err:
            msg = f"Error reading from '{self._filename}': {err.strerror}"

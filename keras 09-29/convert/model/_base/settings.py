...
from convert.model import losses
from tensorflow.keras.optimizers import Adam, Nadam, RMSprop, Optimizer as AdaBelief
from convert.utils import get_backend

# from lib.model.optimizers_plaid import AdaBelief
...
if get_backend() == "amd":
    ...
    from convert.model.autoclip import AutoClipper  # pylint:disable=ungrouped-imports
...
class Optimizer():  # pylint:disable=too-few-public-methods
    ...
    def __init__(self,
        ...
        logger.debug("Initializing %s: (optimizer: %s, learning_rate: %s, autoclip: %s, "
                     ...
                     autoclip, epsilon)
        valid_optimizers = {"adabelief": (AdaBelief,
        ...
        logger.info("Using %s optimizer", optimizer.title())  # type:ignore
        logger.debug("Initialized: %s", self.__class__.__name__)
...
class Settings():
    ...
    def __init__(self,
        ...
        self._set_tf_settings(allow_growth, True) #arguments.exclude_gpus)
    ...
    def _set_tf_settings(cls, allow_growth: bool, exclude_devices: List[int]) -> None:
        ...
        if get_backend() == "cpu":
            logger.info("Hiding GPUs from Tensorflow")  # type:ignore

...
from convert.queue_manager import queue_manager, QueueEmpty
...
class Extractor():
    ...
    def _all_plugins(self):
        ...
        for phase in self._flow:
            ...
            attr = attr[idx] if idx is not None else attr
            retval.append(attr)
        logger.info("All Plugins: %s", retval)
        return retval
    ...
    def _active_plugins(self):
        ...
        logger.info("Active plugins: %s", retval)
        return retval
    ...
    def _load_align(self, aligner, configfile, normalize_method, re_feed):
        ...
        aligner = import_model("extract.align", aligner_name)(exclude_gpus=self._exclude_gpus,
    ...
    def _load_detect(self, detector, rotation, min_size, configfile):
        ...
        detector = import_model("extract.detect", detector_name)(exclude_gpus=self._exclude_gpus,
                                                            rotation=rotation,
                                                            min_size=min_size,
                                                            configfile=configfile,
                                                            instance=self._instance)
    ...
    def _load_mask(self, masker, image_is_aligned, configfile):
        ...
        masker = import_model("extract.mask", masker_name)(exclude_gpus=self._exclude_gpus,
...
class ExtractMedia():
    ...
    def __init__(self, filename, image, detected_faces=None):
        logger.info("Initializing %s: (filename: '%s', image shape: %s, detected_faces: %s)",
    ...
    def get_image_copy(self, color_format):
        ...
        """
        logger.info("Requested color format '%s' for frame '%s'", color_format, self._filename)
    ...
    def add_detected_faces(self, faces):
        ...
        """
        logger.info("Adding detected faces for filename: '%s'. (faces: %s, lrtb: %s)",
    ...
    def remove_image(self):
        ...
        """
        logger.info("Removing image for filename: '%s'", self._filename)
        del self._image
        self._image = None
    ...
    def set_image(self, image):
        ...
        """
        logger.info("Reapplying image: (filename: `%s`, image shape: %s)",

...
from convert.face_filter import FaceFilter as FilterFunc
from convert.image import count_frames, read_image
from convert.utils import (camel_case_split, get_image_paths, _video_extensions)
...
class Images():
    ...
    def _load_video_frames(self):
        ...
        for i, frame in enumerate(reader):
            ...
            logger.info("Loading video frame: '%s'", filename)
    ...
    def load_one_image(self, filename):
        ...
        if self._is_video:
            if filename.isdigit():
                ...
                logger.info("Extracted frame_no %s from filename '%s'", frame_no, filename)
    ...
    def _load_one_video_frame(self, frame_no):
        ...
        """
        logger.info("Loading video frame: %s", frame_no)
        reader = imageio.get_reader(self._args.input_dir, "ffmpeg")
...
class DebugLandmarks(PostProcessAction):  # pylint: disable=too-few-public-methods
    ...
    def process(self, extract_media):
        ...
        for idx, face in enumerate(extract_media.detected_faces):
            logger.info("Drawing Landmarks. Frame: '%s'. Face: %s", frame, idx)
            # Landmarks
    ...
    def process(self, extract_media):
        ...
        for idx, detect_face in enumerate(extract_media.detected_faces):
            ...
            if not self._filter.check(extract_media.image, check_item):
                ...
                continue
            logger.info("Accepting recognised face. Frame: %s. Face: %s",
                         extract_media.filename, idx)

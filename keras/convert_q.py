...
class DiskIO():
    ...
    def _save(self, completion_event):
        ...
        for idx in tqdm(range(self._total_count), desc="Converting", file=sys.stdout):
            ...
            logger.info("Outputting: (filename: '%s'", filename)  # type:ignore
            filenames = [filename.replace(self._args.input_dir, self._args.output_dir)]

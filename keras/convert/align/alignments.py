...
from convert.serializer import get_serializer, get_serializer_from_filename
...
class Alignments():
    ...
    def filter_faces(self, filter_dict, filter_out=False):
        ...
        for source_frame, frame_data in self._data.items():
            ...
            logger.info("frame: '%s', filter_list: %s", source_frame, filter_list)

            for face_idx in reversed(sorted(filter_list)):
                logger.info("Filtering out face: (filename: %s, index: %s)",

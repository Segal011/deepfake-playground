    ...
    def _get_backend(self) -> ValidBackends:
        ...
        while True:
            try:
                ...
                self._configure_backend()
                continue
        fs_backend = "cpu" #config.get("backend", "").lower()
...
def full_path_split(path: str) -> List[str]:
    ...
    while True:
        ...
        path = parts[0]
        allparts.insert(0, parts[1])
    logger.info("path: %s, allparts: %s", path, allparts)  # type:ignore
    return allparts
...
class GetModel():  # pylint:disable=too-few-public-methods
    ...
    def _model_full_name(self) -> str:
        ...
        self.logger.info(retval)  # type: ignore
        return retval
    ...
    def _url_download(self) -> str:
        ...
        self.logger.info("Download url: %s", retval)  # type: ignore
        return retval
